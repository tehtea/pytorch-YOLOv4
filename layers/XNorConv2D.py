...
class XNorConv2D(nn.Module):
    ...
    def forward(self, input):
        ...
        input = F.pad(input, [self.padding[0], self.padding[0], self.padding[1], self.padding[1]], "constant", 1)
        input_binarized = torch.sign(input)
        input_K = F.avg_pool2d(torch.mean(torch.abs(input), 1, keepdim=True, ), self.kernel_size, stride=self.stride,
                               padding=0)
        weight_alpha = torch.abs(self.weight).mean([1, 2, 3], keepdim=True).permute((1, 0, 2, 3))
        if self.training:
            return F.conv2d(input_binarized, torch.sign(self.weight), stride=self.stride, bias=self.bias, padding=0) * \
                   input_K * \
                   weight_alpha
        # if self.training:
        #     return F.conv2d(input_binarized, self.weight, stride=self.stride, bias=self.bias, padding=self.padding)
        else:
